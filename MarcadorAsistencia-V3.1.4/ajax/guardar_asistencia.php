...
include '../../config/conexionPDOcontacto.php';
 $registros=$base->query("SELECT  fecha FROM asistencia WHERE   carnet='$carnet'")->fetchAll(PDO::  FETCH_OBJ); 
...
if($fetq==$hoyo){
  $errors[] = "El alumno ya relizo <br> su asistencia del dia... ";
}
...
// }
		...
		$sql="INSERT INTO asistencia(carnet, fecha) VALUES ('$carnet', '$fecha')";
...
$GUardarA = mysqli_query($con,$sql); /*con la variable $GUardarA efectuo la query*/
		...
		if ($GUardarA){
				...
				$errors []= "Lo siento algo ha salido mal intenta nuevamente.<br> el carnet no coinside con ninguno de nuestra base de datos...";
				//.mysqli_error($con)
			}
